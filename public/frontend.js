...
(() => {
  ...
  function renderPlayers(players) {
    ...
    sorted.forEach((p, index) => {
      ...
      tdLevel.textContent = String(p.level);
    ...
    openModal({
      ...
      bodyHtml: `
        <div class="field">
          ...
          <input id="mp_name" name="name" type="text" value="${escapeHtml(
            currentName
          )}" required />
        </div>
        ...
        <div class="field">
          ...
          <input id="mp_score" name="score" type="number" step="0.5" value="${escapeHtml(
            currentScore ?? 0
          )}" />
        </div>
      `,
      onSubmit: async (fd) => {
        ...
        const score = fd.get("score")?.toString() ?? "0";
  ...
  async function loadSchedules({ keepCurrent = false } = {}) {
    try {
      ...
      list.forEach((s) => addScheduleToList(s));
  ...
  function renderSchedule(schedule) {
    ...
    if (titleEl) titleEl.textContent = "Round Robin Schedule";
    if (metaEl) {
      metaEl.textContent = `Created: ${currentSchedule.createdAt} | Matches: ${currentSchedule.matches.length}`;
    }
  ...
  function applyDateFilter() {
    ...
    const metaEl = document.getElementById("printMeta");
    if (metaEl) metaEl.textContent = `Matches on ${selected} | Count: ${filtered.length}`;
  ...
  function editMatch(m) {
    ...
    openModal({
      ...
      bodyHtml: `
        ...
        <div class="field">
          ...
          <select id="mm_status" name="status">
            ${["scheduled", "completed", "forfeit", "canceled"]
      ...
      onSubmit: async (fd) => {
        ...
        const status = fd.get("status")?.toString() ?? "scheduled";
  ...
  function clearScheduleDisplay() {
    ...
    if (metaEl) metaEl.textContent = "No saved schedules yet.";
